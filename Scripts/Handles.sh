...
#添加CPU温度监控到LuCI（纯controller方案，不用view文件）
CPUTEMP_DIR="./package/base-files/files/usr/lib/lua/luci/controller"
mkdir -p "$CPUTEMP_DIR"
cat > "$CPUTEMP_DIR/cputemp.lua" << 'CPUTEMP'
...
#添加温度JSON接口
cat > "$CPUTEMP_DIR/cputemp_json.lua" << 'CPUTEMPJSON'
